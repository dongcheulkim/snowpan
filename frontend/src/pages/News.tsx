// 스노우판 매거진 목록 (/news) — 홈 "스노우판 매거진" 섹션의 전체 보기. 커뮤니티와 별개 채널(사용자 결정: 커뮤니티 탭이 아니라 홈 섹션).
// 데이터는 community API 의 category=news(관리자 전용 글)를 그대로 쓴다. 작성·수정은 관리자 대시보드 "스노우판 매거진 쓰기".
...
export default function News() {
  useMeta({ title: '스노우판 매거진 | 스노우판', description: '시즌권 판매, 개장일, 리프트권 할인 등 리조트 소식을 스노우판이 정리해 전합니다.' });
    ...
    <div className="max-w-2xl mx-auto space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900">스노우판 매거진</h1>
        </div>
        {isAdmin && <Link to="/community/write?category=news" className="px-3 py-1.5 bg-gray-900 text-white rounded-lg font-bold text-xs">매거진 쓰기</Link>}
      </div>
