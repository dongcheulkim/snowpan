      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-2">
          {shownShops.map((shop) => {
            ...
            return (
            <Link to={shopPath(shop.kind, shop.id, 'repair')} state={{ from: listHere }} key={shop.id} className={`card p-2.5 relative block card-hover ${shop.isPremium ? 'border-sky-300 bg-sky-50/30' : ''}`}>
              ...
              <div className="flex items-center gap-2.5">
                <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
                  {cover
                    ...
                    : <MaintenanceIcon size={22} className="text-gray-300" />}
                </div>
