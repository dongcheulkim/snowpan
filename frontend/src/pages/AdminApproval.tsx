...
const AdminApproval = () => {
  ...
  const renderItem = (item: PendingItem) => {
        ...
        <div key={item.id} className="card p-4">
          <div className="flex items-start gap-3 mb-3">
            <div className="flex-1 min-w-0">
              ...
              {item.address && <div className="text-xs text-gray-500 inline-flex items-center gap-1"><LocationIcon size={12} /> {item.address}</div>}
        ...
        <div key={item.id} className="card p-4">
          <div className="flex items-start gap-3 mb-3">
            <div className="flex-1 min-w-0">
              ...
              {item.user && <div className="text-xs text-gray-500">신청자: {item.user.name}</div>}
              {item.image && (
    ...
    return (
      <div key={item.id} className="card p-4">
        <div className="flex items-start gap-3 mb-3">
          <div className="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-gray-500 flex-shrink-0 overflow-hidden">
            ...
            <div className="flex gap-2 flex-wrap">
              {item.instructorCert && (
                <a href={imageUrl(item.instructorCert)} target="_blank" rel="noopener noreferrer" className="block">
                  <div className="text-[10px] text-gray-500 mb-0.5">강사 자격증</div>
              ...
              {item.businessLicense && (
                <a href={imageUrl(item.businessLicense)} target="_blank" rel="noopener noreferrer" className="block">
                  <div className="text-[10px] text-gray-500 mb-0.5">사업자등록증</div>
              ...
              {item.accommodationPermit && (
                <a href={imageUrl(item.accommodationPermit)} target="_blank" rel="noopener noreferrer" className="block">
                  <div className="text-[10px] text-gray-500 mb-0.5">숙박업 신고증</div>
