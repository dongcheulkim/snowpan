              ...
              <Link key={p.id} to={`/used/${p.id}`} className="card p-3 flex items-center gap-3 card-hover block">
                <div className="w-12 h-12 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0">
                  {(p.image?.startsWith('http') || p.image?.startsWith('/')) ? (
                    <img src={imageUrl(p.image, 200)} alt="" className="w-full h-full object-cover" onError={e => { const i = e.target as HTMLImageElement; if (!i.dataset.fallback) { i.dataset.fallback = '1'; i.src = '/icons/placeholder-card.svg'; } }} />
