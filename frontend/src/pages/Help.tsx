...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: '앱은 어디서 받나요?', a: `${APP_STORE_URL ? `아이폰은 App Store 에서 "스노우판"을 검색하거나 ${APP_STORE_URL} 로 받을 수 있어요.` : '아이폰 앱은 애플 승인이 끝나 곧 App Store 에 올라와요.'} ${PLAY_STORE_URL ? `안드로이드는 Google Play 에서 "스노우판"을 검색하거나 ${PLAY_STORE_URL} 로 받을 수 있어요.` : '안드로이드 앱은 곧 Google Play 에 올라와요.'} 앱 없이도 브라우저에서 "홈 화면에 추가"(아이폰 Safari → 공유, 안드로이드 Chrome → 앱 설치)로 앱처럼 쓸 수 있어요.` },
