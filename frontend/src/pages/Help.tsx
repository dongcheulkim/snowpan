...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: '홈에 있는 스노우판 매거진은 뭐예요?', a: '스노우판 인스타그램(@snowpan.kr)에 올린 소식이 그대로 올라오는 곳이에요. 시즌권, 개장 일정, 장비 이야기를 카드로 넘겨 보고, 누르면 인스타그램에서 전체 내용을 볼 수 있어요.' },
    ],
  },
  {
  ...
  },
  {
    ...
    faqs: [
      { q: '서비스 이용료가 있나요?', a: '매장 등록비와 중개 수수료 없이 운영해요. 광고만 고객센터 채팅으로 상담한 뒤 유료로 진행해요.' },
      { q: '앱은 어디서 받나요?', a: `${APP_STORE_URL ? `아이폰은 App Store 에서 "스노우판"을 검색하거나 ${APP_STORE_URL} 로 받을 수 있어요.` : '아이폰 앱은 애플 승인이 끝나 곧 App Store 에 올라와요.'} ${PLAY_STORE_URL ? `안드로이드는 Google Play 에서 "스노우판"을 검색하거나 ${PLAY_STORE_URL} 로 받을 수 있어요.` : '안드로이드는 브라우저에서 snowpan.kr 로 똑같이 이용할 수 있어요.'} 앱 없이도 브라우저에서 "홈 화면에 추가"(아이폰 Safari → 공유, 안드로이드 Chrome → 앱 설치)로 앱처럼 쓸 수 있어요.` },
