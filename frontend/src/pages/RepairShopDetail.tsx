...
export default function RepairShopDetail() {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      await api('/shop-claims', {
        method: 'POST',
        body: { shopType: 'repair', shopId: shop.id, businessLicense: urls[0], message: claimMsg || undefined },
      });
  ...
  if (!shop) return (
    <div className="text-center py-20">
      ...
      <Link to={backTo} className="text-sm text-sky-600 underline">← 정비샵 목록</Link>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      {!shop.claimable && me && shop.user?.id && shop.user.id !== me.id && (
        <div className="flex gap-2">
          ...
          >방문 예약</button>
          <button
            onClick={() => navigate('/chat/new', { state: { seller: shop.user?.nickname || shop.user?.name || '매장', sellerId: shop.user?.id, productName: shop.name, productImage: shop.image, backTo: `/repair/${shop.id}`, productPath: `/repair/${shop.id}` } })}
