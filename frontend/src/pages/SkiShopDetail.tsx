...
export default function SkiShopDetail() {
  ...
  useMeta({
    ...
    description: shop ? (shop.description?.slice(0, 150) || `${shop.name} 스키·보드샵 정보 - 스노우판`) : undefined,
    ...
    <div className="max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      <ReservationForm
        ...
        onCreated={(roomId) => {
          ...
          navigate(`/chat/${roomId}`, { state: { seller: shop.user?.nickname || shop.user?.name || '매장', sellerId: shop.user?.id, productName: shop.name, productImage: shop.image, backTo: `/skishop/${shop.id}`, productPath: `/skishop/${shop.id}` } });
        }}
      />
