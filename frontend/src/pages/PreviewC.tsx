// PREVIEW — 토스/카카오뱅크 톤 홈 시안. 부드러운 그라데이션·둥근 카드·SVG 아이콘.
// 결정 후 본 Home 에 반영하고 이 파일 삭제.

import { Link } from 'react-router-dom';
import { categoryIcons, SnowRunIcon } from '../components/CategoryIcons';
...
export default function PreviewC() {
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #e0f2fe 0%, #f0f9ff 30%, #ffffff 100%)' }}>
      ...
      {/* Categories — 부드러운 라운드 박스 + SVG 아이콘 */}
      <section className="px-4 pb-4">
          ...
          <div className="grid grid-cols-3 gap-y-4 gap-x-2">
            {CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm text-sky-700"
      ...
      {/* Products — 카드형 그리드 (이모지 X, 이미지 placeholder 회색) */}
      <section className="px-4 py-4">
        ...
        <div className="mt-6 flex justify-center">
          <button
            className="inline-flex items-center gap-2 px-6 py-3 text-xs font-bold text-white rounded-full shadow-md active:scale-95 transition-transform"
