...
// 커뮤니티 종목 선택 — snow 는 스키/보드 (아이콘), 다른 판은 config 의 sports 목록.
  ...
  if (isSnow) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] animate-fade-in px-4">
        ...
        <p className="text-sm text-gray-500 mb-10">종목을 선택해주세요</p>
        ...
        <div className="flex gap-4 w-full max-w-xs">
          ...
          >
            ...
            <span className="text-[11px] text-gray-500">SKI</span>
          </Link>
          ...
          >
            ...
            <span className="text-[11px] text-gray-500">BOARD</span>
          </Link>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col items-center justify-center min-h-[70vh] animate-fade-in px-4">
      ...
      <div className="flex gap-3 w-full max-w-sm">
        {sports.map((s) => (
          <Link
            key={s.id}
            to={`${vbase}/community/${s.id}`}
            className="flex-1 flex flex-col items-center gap-2 py-8 bg-snow border-2 border-gray-200 rounded-2xl active:scale-95 transition-all shadow-sm"
            style={{ color: vertical.accentColor }}
          >
            <span className="text-lg font-bold text-gray-900">{s.label}</span>
            <span className="text-[11px] uppercase tracking-wider" style={{ color: vertical.accentColor }}>{s.id}</span>
          </Link>
        ))}
      </div>
    </div>
