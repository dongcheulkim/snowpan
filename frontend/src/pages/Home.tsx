    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-4 pt-12 pb-4">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="스키, 보드, 장비 검색"
            className="w-full h-11 pl-11 pr-4 rounded-lg text-sm bg-gray-50 border border-gray-100 text-gray-900 placeholder-gray-400"
          />
        </div>
      </div>
        ...
        <div className="flex justify-between">
          {categories.map((cat) => (
            ...
            >
              ...
              <span className="text-[11px] font-medium text-gray-600">{cat.title}</span>
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pt-5 pb-2">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[15px] font-bold text-gray-900">중고 인기매물</h2>
          <Link to="/used" className="text-xs text-gray-400">
            더보기
          </Link>
        </div>
        <div className="space-y-0">
          {hotDeals.map((deal, idx) => (
            <Link
              key={deal.id}
              to={`/used/${deal.id}`}
              className={`flex items-center py-3.5 ${idx !== hotDeals.length - 1 ? 'border-b border-gray-50' : ''}`}
            >
              ...
              <div className="ml-3.5 flex-1 min-w-0">
                ...
                <div className="text-[15px] font-bold text-gray-900 mt-1.5">{deal.price.toLocaleString()}원</div>
              </div>
