...
// 아이콘·이모티콘 없이 글자만(사용자 요청 2026-09-09 "이모티콘은 다 빼자").
...
const categories: Record<string, { label: string; subs: string[] }> = {
  trade:    { label: '거래 관련',    subs: ['상품 문의', '거래 분쟁', '환불/취소', '사기 신고'] },
  account:  { label: '계정/인증',    subs: ['로그인 문제', '비밀번호 변경', '자격증 뱃지', '회원 탈퇴'] },
  business: { label: '사업자 등록',  subs: ['스키·보드샵 등록', '정비샵 등록', '렌탈샵/레슨 등록', '숙소 등록'] },
  ad:       { label: '광고',         subs: ['광고 신청 방법', '광고 비용', '광고 수정/취소', '프리미엄 문의'] },
  report:   { label: '신고/불편',    subs: ['게시글 신고', '유저 신고', '버그/오류', '서비스 건의'] },
  other:    { label: '기타',         subs: ['제휴/협력', '기타 문의'] },
};
...
export default function ChatBotGuide({ onSelect }: Props) {
  ...
  const toggle = () => { if (cat) setSelectedCat(null); else setOpen(v => !v); };
        ...
        <div className="flex items-center justify-between gap-2">
          <button type="button" onClick={toggle} className="min-w-0 text-left" aria-expanded={open}>
            <span className="text-sm font-bold text-gray-900 truncate block">
              {cat ? cat.label : '어떤 도움이 필요하신가요?'}
            </span>
          </button>
          <button type="button" onClick={toggle} className="flex-shrink-0 text-[11px] text-gray-500 px-2 py-1">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {Object.entries(categories).map(([key, c]) => (
              <button
                ...
                className="px-3 py-1.5 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 rounded-full text-xs font-medium text-gray-700 border border-gray-200 transition-colors"
              >
                {c.label}
              </button>
            ))}
