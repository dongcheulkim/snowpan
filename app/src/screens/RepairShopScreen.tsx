...
interface RepairShop {
  ...
  phone?: string; description?: string; services?: string; website?: string;
}

const areas = ['all', '서울', '경기', '강원', '충청', '경상', '전라', '제주'];

export default function RepairShopScreen({ navigation }: any) {
  const [items, setItems] = useState<RepairShop[]>([]);
  ...
  useEffect(() => {
    ...
    api<RepairShop[]>(`/repair-shops?${params}`)
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        ...
        <Text style={s.title}>정비샵 목록</Text>
        <View style={{ width: 32 }} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={s.card}>
            ...
            {item.services && <Text style={s.services}>{item.services}</Text>}
        ...
        ListEmptyComponent={!loading ? <Text style={s.empty}>정비샵 정보가 없습니다.</Text> : null}
      />
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  services: { fontSize: 12, color: '#38BDF8', fontWeight: '500', marginTop: 4 },
