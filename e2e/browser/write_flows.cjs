...
(async () => {
  ...
  const prodOk = await waitText(p, new RegExp(`점검 매물 ${ts}`), 8000) || /\/used\/[0-9a-f-]{36}/.test(p.url()); ok('중고 등록', prodOk, p.url().replace(BASE, '') + ' ' + (await txt(p)).replace(/\s+/g, ' ').slice(0, 100));
