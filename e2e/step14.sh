...
api GET "/community?sport=ski" ""; NC4=$(echo "$RESP" | jq -r "[.posts[] | select(.id==\"$NP\")] | length"); [ "$NC4" = "0" ] && ok "기본 커뮤니티 목록에는 스키장 소식 안 섞임(홈·/news 전용)" || bad "기본 목록 news=$NC4"
api GET "/community?sport=board&category=news" ""; NC3=$(echo "$RESP" | jq -r "[.posts[] | select(.id==\"$NP\")] | length"); [ "$NC3" = "1" ] && ok "category=news 명시 시 보드 종목에서도 조회(공용)" || bad "보드 노출 n=$NC3"
